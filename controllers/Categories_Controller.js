...
const updateCategory = async (req, res) => {
  const { categoryname } = req.body;
  const { firstname, user_role } = req.obj;
  ...
  try {
    ...
    const changeCategoryNameQuery = await sequelize.query(
      "UPDATE `categories` SET `categoryname` = ? WHERE `categories`.`id` = ?",
      {
        ...
        replacements: [categoryname.toLowerCase(), categoryDetail.id],
      }
    );
    ...
    res.status(200).json({
      status: "success",
      msg: `'${categoryDetail.categoryname}' is changed to '${categoryname}'`,
    });
  } catch (error) {
...
const deleteCategory = async (req, res) => {
  const { firstname, user_role, gender } = req.obj;
  try {
    const categoryId = req.params.id;
    const { id } = req.obj;
    ...
    if (user_role != 1) {
      ...
      if (id != categoryDetail.createdBy) {
        return res.status(400).json({
          error: `DELETION ABORTED, Category does not belong's to you ${holder + firstname
            }`,
        });
      }
    }
